...
export function DeveloperJourneyFlow() {
  ...
  useEffect(() => {
    ...
    const height = wrapperRef.current.clientHeight || wrapperRef.current.clientWidth * 0.6
  ...
  return (
    <Card className="p-4">
      ...
      <div className="text-sm text-gray-600 mb-4">
        Interactive visualization of the developer journey through different stages,
        showing touchpoints, impact, and progression
      </div>
      ...
      <div className="mt-4 text-sm text-gray-600">
        <p className="mb-2">Journey stages represent key phases in the developer experience:</p>
        <ul className="list-disc ml-6 space-y-1">
          {journeyStages.map((stage, index) => (
            <li key={index} className="line-clamp-2">
              <span className="font-semibold">{stage.name}:</span> {stage.description}
            </li>
          ))}
        </ul>
      </div>
